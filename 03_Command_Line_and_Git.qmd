...
Only use letters (`a-z`, `A-Z`), numbers (`0-9`), underscores (`_`), and hyphens (`-`) in your file and folder names. This will make navigating your file system from the command line much smoother. It is also good practice to have a consistent [naming system](https://en.wikipedia.org/wiki/Naming_convention_(programming)).
...
-   Instead of storing changes to files across versions, Git stores snapshots of the project over time. The below figures from @chacon_pro_2020 provide a visualization of what saving changes would look like and how Git stores data. Effectively, a picture of the whole project is taken each time you "save" your project.

```{r echo = FALSE, fig.cap = "Storing Changes", fig.align = "center", out.width = "80%", fig.alt = "A diagram titled `Checkins Over Time` showing five versions labeled Version 1 through Version 5. Below each version are three rows labeled File A, File B, and File C. Arrows connect labeled change blocks such as Δ1, Δ2, and Δ3 across versions, indicating updates to each file between versions."}
knitr::include_graphics("Figures/Chacon-Straub_Fig4.png")
```

```{r echo = FALSE, fig.cap = "Storing Snapshots", fig.align = "center", out.width = "80%", fig.alt="A diagram titled Checkins Over Time showing five versions labeled Version 1 through Version 5. Each version contains vertical stacks for File A, File B, and File C, with boxes labeled A1, A2, B, B1, C1, C2, and C3 representing file states. Some boxes are reused or repeated across versions, and some are shown as dotted outlines."}
...
```{r echo = FALSE, fig.cap = "Storing Snapshots", fig.align = "center", out.width = "80%", fig.alt= "A diagram showing four file states in Git: untracked, unmodified, modified, and staged. The diagram illustrates how a file moves between these states. A file is added to move from untracked to unmodified. Editing the file changes it from unmodified to modified. Staging the file moves it from modified to staged. Committing the file brings it back to unmodified. A file can also be removed to return from unmodified to untracked."}
...
The command `git log` allows you to view the commit history of your repository. See @chacon_pro_2020 for details on how to format the output of this. Another option is to view the repository on GitHub's website.
...
[^2]: See @zell_what_2015 for a helpful explanation.
...
When starting a project and setting up a repository, you can reference [this list](https://github.com/github/gitignore) to populate your `.gitignore`. Checking `git status` frequently will help you keep the `.gitignore` updated and useful.
...
4.  From the command line, create a file called `README.md`. Write whatever you would like to describe the repository. If in doubt, write: `This repository contains my assingments for the Summer 2022 R and workflow minicourse.`
...
### "I want to discard my changes to a file since the last commit."
