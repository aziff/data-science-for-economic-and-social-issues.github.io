...
1.  Define a vector `x` of 10 random draws from the uniform $[0,1]$ distribution. Test if each element is greater than or equal to 0.9. Use the command `runif()`.
...
1.  Define an object `out` with the value 0. In a `for` loop iterating 1, 2, \ldots, 20, add the reciprocal to `out` if the number is even. The sum should be $\frac{1}{2} + \frac{1}{4} + \ldots + \frac{1}{20}$.
