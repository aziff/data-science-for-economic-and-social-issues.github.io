...
2.  Download the dataset `GSS2022.dta` to your computer. It is on Canvas \> Modules \> Data. Open the dataset in R using `read_dta()` from the package `haven`.

3.  Make a bar plot of the variable `tax`. Refer to the documentation to learn what this variable means. Make sure you look for the place in the PDF codebook where it lists what the values mean.

4.  Edit your plot from 3 to have labels for the values of `tax`. To do this, you can create a new variable `tax_str` that takes the values "Too High", "About Right", "Too Low". It can be a character variable or a factor variable. Hint: It is easiest to do it with a factor variable using the function `factor`.
...
8.  Let's try to graphically answer the following question: Does political party correlate with opinions on the federal income tax?. Create a bar plot that graphs the count of responses to `tax` for each level of `partyid`. Use the argument `position = "dodge"` in `geom_bar()`. The x-axis should be the values of `partyid` and there should be three bars for each value of `tax_str` corresponding to the party identification.
...
10. Make a bar plot of the response to the variable you created in question 9 by `tax_str` . The x-axis should be the values of `tax_str` and there should be two or three bars for each value corresponding to the parties.
